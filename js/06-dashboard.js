...
function renderDashboard(r){
  ...
  const tenantPill=`<div class="tenant-pill" onclick="Router.go('#/'+'${shop.slug}')"><div class="tl">${initials(shop.name)}</div><div class="info"><b>${escapeHtml(shop.name)}</b><span>${DB.find('plans',shop.planId).name} · /${escapeHtml(shop.slug)}</span></div>${icon('eye')}</div>`;
  $('#root').innerHTML=mountShell({brandSub:'Gestão',nav:buildDashNav(shop),activeId:sub,navBase:'#/dashboard/',title:titles[sub]||'Painel',crumb:shop.name+' · '+ROLE_LABEL[Session.effectiveUser.role],content,tenantPill});
...
function delBarber(id){confirmAction('Excluir barbeiro?','Esta ação não pode ser desfeita.',()=>{DB.remove('barbers',id);toast('Barbeiro excluído.','info');refreshShell();});}
...
function saveConfig(){const shop=dashShop();DB.update('barbershops',shop.id,{name:$('#cf_name').value.trim(),description:$('#cf_desc').value.trim(),address:$('#cf_addr').value.trim(),city:$('#cf_city').value.trim(),neighborhood:$('#cf_neigh').value.trim(),instagram:$('#cf_ig').value.trim(),phone:$('#cf_phone').value.trim(),whatsapp:$('#cf_wa').value.trim(),open:$('#cf_open').value,close:$('#cf_close').value,lunchStart:$('#cf_ls').value,lunchEnd:$('#cf_le').value,slotInterval:+$('#cf_int').value});DB.log('Configurações atualizadas',shop.name,shop.id);toast('Configurações salvas.','ok');refreshShell();}
...
function renderBarber(){
  ...
  $('#root').innerHTML=mountShell({brandSub:'Barbeiro',nav,activeId:'',navBase:'#/my-schedule/',title:'Minha agenda',crumb:shop.name+' · '+barber.name,content,search:false});
  renderShellNotif();
}
